...
| End-to-end orchestrator | `src/run_all.py` | Single-command runner that executes every production pipeline in dependency order (detect → color / pattern / CLIP refine → join → Pipeline 1 visualisations → Pipeline 2). Each step re-uses its own per-pipeline YAML config; failures abort the run so downstream stages don't compound stale inputs. The interactive validation app is deliberately excluded |
